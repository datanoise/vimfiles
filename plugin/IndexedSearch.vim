...
function! s:ScheduleEchoEvent()
  let &ut = g:IndSearchUT
  if bufname('%') != "ControlP"
    if s:DelaySearchIndex
      call s:ShowCurrentSearchIndex(0,'')
      let s:ScheduledEcho = s:Msg
      let s:ScheduledHighlight = s:Highlight
      let s:DelaySearchIndex = 0
    endif
    if s:ScheduledEcho != ""
      exe "echohl ".s:ScheduledHighlight
      echo s:ScheduledEcho
      echohl None
      let s:ScheduledEcho=''
    endif
  endif
...
func! s:ScheduleEcho(msg,highlight)
    ...
    au CursorHold * :call s:ScheduleEchoEvent()
