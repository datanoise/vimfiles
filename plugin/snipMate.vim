...
fun! MakeSnip(scope, trigger, content, ...)
	...
	elseif multisnip | let {var}[a:scope][a:trigger] += [[a:1, a:content]]
		...
		if !s:ignore_dups
			echom 'Warning in snipMate.vim: Snippet '.a:scope.':'.a:trigger.' is already defined.'
...
fun! ResetSnippets()
	let s:snippets = {} | let s:multi_snips = {} | let s:did_ft = {}
endf

let s:did_ft = {}
let g:snipmate_aliases = {
			\ 'objc': ['c'],
			\ 'cpp':  ['c'],
			\ 'cs':   ['c'],
			\ 'xhtml': ['html'],
...
fun! GetSnippets(dir, filetypes)
	for ft in split(a:filetypes, '\.')
		...
		if has_key(g:snipmate_aliases, ft)
			let scopes = g:snipmate_aliases[ft]
			let s:ignore_dups = 1
			for scope in scopes
				call s:DefineSnips(a:dir, scope, ft)
			endfor
			let s:ignore_dups = 0
		endif
		let s:did_ft[ft] = 1
